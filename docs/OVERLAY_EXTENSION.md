...
- **P1 — Q2 skeleton.** Manifest, popup (console URL), dynamic content-script
  registration, background worker, an anchored card that lists live per-AP client
  counts from the same-origin APIs. *No DOM capture needed for a floating card.*
- **P2 — Q2 map.** Render the floor image + AP markers + live client dots in the
  card (port the dashboard client-map renderer); floor/site picker.
- **P3 — Q2 in-map overlay (if SVG).** After the DOM capture: either inject dots
  into InnerSpace's SVG (auto-aligned) or keep the card.
...
- **P5 — Plan overlay.** Bridge `GET /api/plan` + plan⟷live toggle; optional
  Hamina share-image embed; "Open in Hamina" deep link.

## What I still need from you

1. **InnerSpace DOM snapshot** (F12 → Elements → Copy outerHTML of `<body>`, or a
   screenshot of the Elements panel around the map) — anchor point + SVG-vs-canvas.
2. **Heatmap?** If Hamina's computed heatmap picture matters, we add the Hamina
   share-link source in P5; otherwise vector plan-vs-live is enough.
3. **Repo home:** this `browser-extension/` dir inside `unifi-hamina-live` (chosen
   default — the bridge is its data source), or a separate repo?
4. **Chrome only or Firefox too?** (MV3 is Chrome-first.)
